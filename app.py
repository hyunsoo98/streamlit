...
# --- 로그인 페이지로 이동하는 버튼 ---
# 'pages/page_2.py' (로그인 페이지)로 이동
st.page_link("pages/page_2.py", label="로그인 또는 회원가입", icon="🔑")

# --- 이미지 분석 페이지로 바로 이동하는 버튼 (선택 사항) ---
# 로그인이 필요 없다면 이 버튼으로 바로 핵심 기능에 접근 가능
st.page_link("pages/page_1.py", label="로그인 없이 이미지 분석 시작하기", icon="🚀")
